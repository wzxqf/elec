> version: v0.42
> experiment_id: v0.42-d0f1cd29
> config_hash: d0f1cd29e630e124c05e6cf7194aae745f54b32a4507faee6bb7207d66a63e29
> run_timestamp: 2026-04-17T15:51:38+08:00
> device: cuda
> data_range: 2025-11-01 00:00:00 -> 2026-03-20 23:45:00

# 运行总结

- 版本: v0.42
- 算法: HYBRID_PSO_V040
- 训练设备: cuda
- 验证成本: 4912195328.00
- 验证收益: 1083457792.00
- 验证 CVaR99: 2970814.88
- 滚动窗口数: 10
- 平均回测成本: 1392841344.00
- 平均回测收益: 171983545.20
- 平均回测 CVaR99: 3139273.83
- 持续跑赢窗口数: 7
- 模块1摘要: reports/module1_summary.md
- 市场机制分析: reports/market_mechanism_analysis.md
- 超额收益验证摘要: reports/excess_return_validation_summary.md

