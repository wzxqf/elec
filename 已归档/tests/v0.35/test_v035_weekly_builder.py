...
def _build_frame() -> pd.DataFrame:
    ...
    id_price[(dt >= pd.Timestamp("2025-11-10 12:00:00")) & (dt < pd.Timestamp("2025-11-10 16:00:00"))] += 50.0
    return pd.DataFrame(
        {
            "datetime": dt,
...
def test_build_weekly_bundle_emits_v035_weekly_fields() -> None:
    ...
    assert "da_id_cross_corr_w" in weekly_metadata.columns
    assert "extreme_event_flag_w" in weekly_metadata.columns
    assert "extreme_price_spike_flag_w" in weekly_metadata.columns
    assert weekly_metadata["da_id_cross_corr_w"].notna().all()
