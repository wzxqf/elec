...
def test_hourly_score_kernel_uses_lagged_decision_signals() -> None:
    ...
    bundle["hourly"]["price_spread"] = [200.0, 200.0]
    bundle["hourly"]["price_spread_lag1"] = [0.0, 0.0]
    bundle["hourly"]["load_dev_lag1"] = [0.0, 0.0]
    bundle["hourly"]["renewable_dev_lag1"] = [0.0, 0.0]
    bundle["hourly"]["price_spread_abs_lag1"] = [0.0, 0.0]
    bundle["hourly"]["renewable_dev_abs_lag1"] = [0.0, 0.0]
    bundle["feasible_domain"] = compile_feasible_domain(
        config=config,
        weekly_metadata=bundle["weekly_metadata"],
        policy_state_trace=bundle["policy_state_trace"],
...
def test_contract_curve_changes_interval_procurement_cost() -> None:
    week = pd.Timestamp("2026-02-02")
    hours = list(range(24))
    intervals = list(range(96))
    bundle = {
        "weekly_features": pd.DataFrame({"week_start": [week], "feature_a": [0.0], "feature_b": [0.0]}),
        "weekly_metadata": pd.DataFrame(
            {
                "week_start": [week],
                "forecast_weekly_net_demand_mwh": [2400.0],
                "actual_weekly_net_demand_mwh": [2400.0],
                "lt_price_w": [100.0],
                "hour_count": [24],
            }
        ),
        ...
        "hourly": pd.DataFrame(
            {
                "week_start": [week] * 24,
                "hour_index": hours,
                "net_load_da": [100.0] * 24,
                "net_load_id": [100.0] * 24,
                "price_spread": [0.0] * 24,
                "load_dev": [0.0] * 24,
                "renewable_dev": [0.0] * 24,
            }
        ),
        "quarter": pd.DataFrame(
            {
                ...
                "全网统一出清价格_日前": [800.0 if interval < 48 else 50.0 for interval in intervals],
                "全网统一出清价格_日内": [800.0 if interval < 48 else 50.0 for interval in intervals],
            }
        ),
        "agent_feature_columns": ["feature_a", "feature_b"],
    }
    config = _config()
    config["score_kernel"] = {
        "contract_position_base_ratio": 1.00,
    ...
    layout = compile_parameter_layout(config=config, bundle=bundle)
    tensor_bundle = compile_training_tensor_bundle(bundle, device="cpu")
    curve_block = next(block for block in layout.upper.blocks if block.name == "contract_curve_latent")
    early = torch.zeros((1, layout.upper.total_dimension), dtype=torch.float32)
    late = torch.zeros((1, layout.upper.total_dimension), dtype=torch.float32)
    early[0, curve_block.slice_start + 1] = 6.0
    late[0, curve_block.slice_start + 1] = -6.0
    lower = torch.zeros((1, layout.lower.total_dimension), dtype=torch.float32)

    early_result = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=early,
        lower_particles=lower,
        device="cpu",
        config=config,
        compiled_layout=layout,
    )
    late_result = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=late,
        lower_particles=lower,
        device="cpu",
        config=config,
        compiled_layout=layout,
    ...
    assert early_result.weekly_procurement_cost[0, 0, 0] > late_result.weekly_procurement_cost[0, 0, 0]


def test_contract_curve_guard_uniforms_linked_policy_weeks() -> None:
    bundle = _bundle()
    config = _config()
    config["score_kernel"] = {
        "contract_position_base_ratio": 0.60,
    ...
    curve_block = next(block for block in layout.upper.blocks if block.name == "contract_curve_latent")
    upper = torch.zeros((1, layout.upper.total_dimension), dtype=torch.float32)
    upper[0, curve_block.slice_start + 1] = 6.0
    lower = torch.zeros((1, layout.lower.total_dimension), dtype=torch.float32)
    ...
    expected = torch.full((24,), 1.0 / 24.0, dtype=torch.float32)
    assert torch.allclose(result.contract_curve[0, 0, 0], expected, atol=1.0e-6)


def test_contract_curve_guard_leaves_inactive_policy_weeks_unchanged() -> None:
    bundle = _bundle()
    bundle["policy_state_trace"]["lt_price_linked_active"] = [0.0]
    config = _config()
    config["score_kernel"] = {
        "contract_position_base_ratio": 0.60,
        "exposure_band_base_ratio": 0.00,
    ...
    base_result = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=upper,
        lower_particles=lower,
        device="cpu",
    ...
    guarded_result = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=upper,
        lower_particles=lower,
        device="cpu",
    ...
    assert torch.allclose(guarded_result.contract_curve, base_result.contract_curve)


def test_lock_deviation_penalty_is_noop_when_configured_zero() -> None:
    bundle = _bundle()
    base_config = _config()
    base_config["score_kernel"] = {
        "contract_position_base_ratio": 0.60,
        "contract_adjustment_scale_ratio": 0.30,
        "contract_adjustment_feature_scale": 0.00,
        "contract_adjustment_policy_scale": 0.00,
        "exposure_band_base_ratio": 0.00,
    ...
    layout = compile_parameter_layout(config=base_config, bundle=bundle)
    tensor_bundle = compile_training_tensor_bundle(bundle, device="cpu")
    upper = torch.zeros((1, layout.upper.total_dimension), dtype=torch.float32)
    lower = torch.zeros((1, layout.lower.total_dimension), dtype=torch.float32)
    action_head = next(block for block in layout.upper.blocks if block.name == "action_head")
    upper[0, action_head.slice_start] = 0.75
    ...
    zero_penalty_config = {
        **base_config,
        "reward": {**base_config["reward"], "lambda_lock_deviation": 0.0},
    }
    zero_penalty_result = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=upper,
        lower_particles=lower,
        device="cpu",
        config=zero_penalty_config,
        compiled_layout=layout,
    )

    assert base_result.contract_adjustment_mwh_exec[0, 0, 0] > 0.0
    assert torch.allclose(zero_penalty_result.reward, base_result.reward)
    assert torch.allclose(zero_penalty_result.total_score, base_result.total_score)


def test_positive_lock_deviation_penalty_only_applies_to_upward_adjustment() -> None:
    bundle = _bundle()
    base_config = _config()
    base_config["score_kernel"] = {
        "contract_position_base_ratio": 0.60,
    ...
    action_head = next(block for block in layout.upper.blocks if block.name == "action_head")
    lower = torch.zeros((1, layout.lower.total_dimension), dtype=torch.float32)
    positive_upper = torch.zeros((1, layout.upper.total_dimension), dtype=torch.float32)
    negative_upper = torch.zeros((1, layout.upper.total_dimension), dtype=torch.float32)
    positive_upper[0, action_head.slice_start] = 0.75
    negative_upper[0, action_head.slice_start] = -0.75
    penalty_config = {
        **base_config,
        "reward": {**base_config["reward"], "lambda_positive_lock_deviation": 1_000_000.0},
    }

    positive_base = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=positive_upper,
        lower_particles=lower,
        device="cpu",
        config=base_config,
        compiled_layout=layout,
    )
    positive_penalty = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=positive_upper,
        lower_particles=lower,
        device="cpu",
    ...
    negative_base = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=negative_upper,
        lower_particles=lower,
        device="cpu",
        config=base_config,
        compiled_layout=layout,
    )
    negative_penalty = batch_score_particles(
        tensor_bundle=tensor_bundle,
        upper_particles=negative_upper,
        lower_particles=lower,
        device="cpu",
    ...
    assert positive_base.contract_adjustment_mwh_exec[0, 0, 0] > 0.0
    assert positive_penalty.reward[0, 0] < positive_base.reward[0, 0]
    assert negative_base.contract_adjustment_mwh_exec[0, 0, 0] < 0.0
    assert torch.allclose(negative_penalty.reward, negative_base.reward)
    assert torch.allclose(negative_penalty.total_score, negative_base.total_score)
...
def test_hourly_hedge_uses_projected_exposure_floor_for_limit() -> None:
    bundle = _bundle()
    config = _config()
    config["score_kernel"] = {
        "contract_position_base_ratio": 0.60,
        "exposure_band_base_ratio": 0.00,
        "exposure_band_feature_scale": 0.00,
        "hourly_signal": {
            "spread_weight": 0.50,
            "load_dev_weight": 0.00,
            "renewable_weight": 0.00,
    ...
    assert result.exposure_band_mwh_raw[0, 0, 0] == 0.0
    assert result.exposure_band_mwh[0, 0, 0] > 0.0
    assert torch.count_nonzero(result.spot_hedge_mwh.abs() > 1.0e-6).item() > 0
    assert result.weekly_friction_cost[0, 0, 0] > 0.0
