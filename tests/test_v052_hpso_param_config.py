...
def test_sections_are_normalized() -> None:
    ...
    assert hybrid_pso["upper"]["dimension"] == 182
...
def test_runtime_sections_include_hybrid_optimizer_and_score_kernel() -> None:
    ...
    assert config["reward"]["baseline_position_ratios"] == [0.48]
    assert config["reward"]["lambda_lock_deviation"] == 0.0
    assert config["reward"]["lambda_positive_lock_deviation"] == 10_000_000_000.0
    ...
    assert score_kernel["contract_position_base_ratio"] == 0.48
    assert score_kernel["baseline_position_ratio"] == 0.48
    ...
    weekly_scaling = score_kernel["weekly_feature_scaling"]
    assert weekly_scaling["enabled"] is True
    assert weekly_scaling["price_scale_yuan_per_mwh"] == 500.0
    assert weekly_scaling["forward_days_scale"] == 60.0
    assert weekly_scaling["clip_abs"] == 5.0
    assert score_kernel["contract_curve_guard"]["enabled"] is True
    assert score_kernel["contract_curve_guard"]["policy_column"] == "lt_price_linked_active"
    assert score_kernel["contract_curve_guard"]["uniform_blend"] == 0.85
    assert score_kernel["contract_curve_guard"]["also_bind_feasible_domain"] is False
    ...
    assert score_kernel["hourly_limit"]["shrink_multiplier"] == 0.00
    assert score_kernel["hourly_gate"]["mode"] == "hard"
    assert score_kernel["hourly_gate"]["signal_deadband"] == 0.05
