...
from src.scripts import hourly_spot_param_search
    ...
    frame = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "hourly_limit_base_multiplier": 0.50,
                "hourly_limit_shrink_multiplier": 0.00,
                "friction_cost_yuan_per_mwh": 1.50,
                "lambda_trade": 0.10,
    ...
    baseline = derive_hourly_spot_baseline(
        ...
        config={
            "score_kernel": {
                ...
                "hourly_gate": {"mode": "hard", "signal_deadband": 0.05, "temperature": 0.04},
                "hourly_limit": {"base_multiplier": 0.50, "shrink_multiplier": 0.00},
            },
...
def test_param_search_parallel_evaluator_preserves_all_candidates(monkeypatch: pytest.MonkeyPatch) -> None:
    candidates = build_hourly_spot_experiment_grid(
        {
            "hourly_spot_experiment": {
                "enabled": True,
    ...
    def fake_evaluate_candidate(*, context, snapshots, candidate):
        return {
            "experiment_id": candidate.experiment_id,
            "signal_transform": candidate.signal_transform,
            "gate_mode": candidate.gate_mode,
            "signal_clip_abs": candidate.signal_clip_abs,
            "sum_excess_profit_w": float(candidate.signal_deadband),
            "mean_cvar99_w": 1.0,
            "mean_hedge_error_w": 1.0,
            "nonzero_hour_share": 1.0,
            "spot_abs_sum_mwh": 1.0,
        }

    monkeypatch.setattr(hourly_spot_param_search, "_evaluate_candidate", fake_evaluate_candidate)

    rows = hourly_spot_param_search._evaluate_candidates(
        context={},
    ...
    assert [row["experiment_id"] for row in rows] == ["hourly_spot_001", "hourly_spot_002"]
