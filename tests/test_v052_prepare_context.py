...
def test_manifest_key_output_merge_preserves_full_pipeline_outputs() -> None:
    output_root = Path(".cache") / "tests" / f"manifest-merge-{uuid4().hex}" / "outputs" / "v0.52"
    ...
    (reports_dir / "v0.52_human_report.md").write_text("# report\n", encoding="utf-8")
    ...
    assert merged["human_report_path"] == "reports/v0.52_human_report.md"
