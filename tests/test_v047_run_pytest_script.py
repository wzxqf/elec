...
def test_invoke_pytest_uses_subprocess_with_pytest_module_and_no_bytecode_env(monkeypatch) -> None:
    ...
    code = module.invoke_pytest(["tests/test_v047_hpso_param_config.py", "-q"])

    assert code == 0
    assert captured["command"] == [sys.executable, "-m", "pytest", "tests/test_v047_hpso_param_config.py", "-q"]
