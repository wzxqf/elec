...
def test_train_hybrid_pso_model_emits_iteration_progress() -> None:
    tensor_bundle = compile_training_tensor_bundle(_bundle(), device="cpu")
    config = {
        "training": {"device": "cpu", "algorithm": "HYBRID_PSO_V040"},
        "reward": {
    ...
    progress_events: list[dict[str, float]] = []
    train_hybrid_pso_model(tensor_bundle=tensor_bundle, config=config, progress_callback=progress_events.append)

    assert len(progress_events) == 3
    assert progress_events[0]["iteration"] == 1
    assert progress_events[-1]["iteration"] == 3
    assert progress_events[0]["iterations"] == 3
    assert progress_events[-1]["phase_progress"] == 1.0
    assert progress_events[0]["phase_progress"] < progress_events[-1]["phase_progress"]
    assert "best_score" in progress_events[-1]
    assert "mean_score" in progress_events[-1]
