...
def test_source_archive_excludes_local_outputs_caches_git_and_secret_files() -> None:
    ...
    try:
        ...
        (project_root / "tests" / "test_v052_example.py").write_text("def test_ok():\n    assert True\n", encoding="utf-8")
        (project_root / "outputs" / "v0.52").mkdir(parents=True)
        (project_root / "outputs" / "v0.52" / "local.csv").write_text("local\n", encoding="utf-8")
        (project_root / "已归档" / "outputs" / "v0.44-param-opt-explore" / "logs").mkdir(parents=True)
        (project_root / "已归档" / "outputs" / "v0.44-param-opt-explore" / "logs" / "old.log").write_text(
            "archived output\n",
            encoding="utf-8",
        )
        ...
        assert "tests/test_v052_example.py" in names
        assert "outputs/v0.52/local.csv" not in names
        assert "已归档/outputs/v0.44-param-opt-explore/" in names
        assert "已归档/outputs/v0.44-param-opt-explore/logs/" in names
        assert "已归档/outputs/v0.44-param-opt-explore/logs/old.log" not in names
...
def test_remote_execution_code_runs_pipeline_then_pytest_and_emits_result_marker() -> None:
    spec = run_remote_jupyter.RemoteRunSpec(
        run_id="v052-remote-test",
        version="v0.52",
        archive_remote_path="elec_remote_runs/v0.52/v052-remote-test/source.zip",
        remote_project_dir="elec_remote_runs/v0.52/v052-remote-test/project",
        artifact_remote_path="elec_remote_runs/v0.52/v052-remote-test/artifacts.zip",
        pytest_args=["tests/test_v052_repo_cleanup.py", "-q"],
        run_pipeline=True,
        run_tests=True,
    )
    ...
    assert '"tests/test_v052_repo_cleanup.py"' in code
    assert '"-q"' in code
    assert "CONFIG = json.loads(" in code
    compile(code, "<remote_execution>", "exec")


def test_remote_execution_code_accepts_custom_python_commands() -> None:
    spec = run_remote_jupyter.RemoteRunSpec(
        run_id="v052-param-search",
        version="v0.52",
        archive_remote_path="elec_remote_runs/v0.52/v052-param-search/source.zip",
        remote_project_dir="elec_remote_runs/v0.52/v052-param-search/project",
        artifact_remote_path="elec_remote_runs/v0.52/v052-param-search/artifacts.zip",
        pytest_args=[],
        run_pipeline=False,
        run_tests=False,
        custom_commands=[
            {
                ...
                "argv": [
                    "-m",
                    "src.scripts.hourly_spot_param_search",
                    "--project-root",
                    ".",
                    "--workers",
                    "8",
    ...
    assert '"custom_commands"' in code
    assert '"param_search"' in code
    assert '"src.scripts.hourly_spot_param_search"' in code
    assert '"--workers"' in code
    compile(code, "<remote_execution>", "exec")


def test_parse_args_accepts_remote_command_json() -> None:
    args = run_remote_jupyter.parse_args(
        [
            "--dry-run",
            "--remote-command-json",
            '{"name":"param_search","argv":["-m","src.scripts.hourly_spot_param_search","--workers","4"]}',
        ]
    )

    assert args.remote_command_json == [
        '{"name":"param_search","argv":["-m","src.scripts.hourly_spot_param_search","--workers","4"]}'
    ]
...
def test_default_download_dir_is_versioned_remote_jupyter_subdirectory() -> None:
    ...
    download_dir = run_remote_jupyter.default_download_dir(project_root, "v0.52", "20260426T120000")

    assert download_dir == project_root / "outputs" / "v0.52" / "raw" / "remote_jupyter" / "20260426T120000"
...
def test_sync_pulled_outputs_replaces_local_formal_outputs_and_keeps_remote_history() -> None:
    ...
    try:
        project_root = temp_root / "project"
        target = project_root / "outputs" / "v0.52"
        target.mkdir(parents=True)
        (target / "raw" / "metrics").mkdir(parents=True)
        (target / "raw" / "metrics" / "old.csv").write_text("old\n", encoding="utf-8")
        (target / "stale_manifest.json").write_text("{}\n", encoding="utf-8")
        remote_history = target / "raw" / "remote_jupyter" / "run-1"
        remote_history.mkdir(parents=True)
        (remote_history / "keep.txt").write_text("history\n", encoding="utf-8")

        pulled_output = remote_history / "outputs" / "v0.52"
        (pulled_output / "raw" / "metrics").mkdir(parents=True)
        (pulled_output / "reports").mkdir()
        (pulled_output / "raw" / "metrics" / "new.csv").write_text("new\n", encoding="utf-8")
        (pulled_output / "reports" / "summary.md").write_text("# remote\n", encoding="utf-8")
        (pulled_output / "artifact_index.md").write_text("remote index\n", encoding="utf-8")

        manifest = run_remote_jupyter.sync_pulled_outputs_to_local(remote_history, project_root, "v0.52")

        assert not (target / "raw" / "metrics" / "old.csv").exists()
        assert not (target / "stale_manifest.json").exists()
        assert (target / "raw" / "metrics" / "new.csv").read_text(encoding="utf-8") == "new\n"
        ...
        assert manifest["preserved_names"] == ["raw/remote_jupyter"]
    finally:
        shutil.rmtree(temp_root, ignore_errors=True)
...
def test_root_remote_jupyter_script_delegates_to_module_dry_run() -> None:
    ...
    assert "Remote root: elec_remote_runs/v0.52/root-wrapper-test" in result.stdout


def test_powershell_remote_jupyter_launcher_is_one_click_safe() -> None:
    script_path = PROJECT_ROOT / "run_remote_jupyter.ps1"

    text = script_path.read_text(encoding="utf-8")

    assert "run_remote_jupyter.py" in text
    assert "D:\\miniforge\\envs\\torch311\\python.exe" in text
    assert "ELEC_JUPYTER_URL" in text
    assert "ELEC_JUPYTER_KERNEL" in text
    assert "ELEC_REMOTE_ENV" in text
    assert "--probe" in text
    assert "ELEC_JUPYTER_PASSWORD=\"" not in text
    assert "ELEC_JUPYTER_TOKEN=\"" not in text
