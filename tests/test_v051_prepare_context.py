...
def test_manifest_key_output_merge_preserves_full_pipeline_outputs() -> None:
    output_root = Path(".cache") / "tests" / f"manifest-merge-{uuid4().hex}" / "outputs" / "v0.51"
    ...
    assert merged["human_report_path"] == "reports/v0.51_human_report.md"
