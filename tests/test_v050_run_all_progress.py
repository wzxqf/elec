...
def test_run_pipeline_honors_runtime_status_env(monkeypatch) -> None:
    ...
    for path in [metrics_dir, metadata_dir, models_dir, logs_dir, reports_dir]:
    ...
    def fake_prepare_project_context(*args, **kwargs):
        return {
            "config": {"version": CURRENT_PROJECT_VERSION, "training": {"algorithm": "HYBRID_PSO_V040"}},
            "output_paths": {
                "root": output_root,
                "raw": raw_dir,
                "reports": reports_dir,
    ...
    def fake_run_train(context):
        ...
        assert payload["phase_name"] == "HYBRID_PSO_V040 训练"
    ...
    assert sorted(path.name for path in reports_dir.iterdir()) == [
        f"{CURRENT_PROJECT_VERSION}_ai_structured_report.json",
        f"{CURRENT_PROJECT_VERSION}_human_report.md",
    ]
...
def test_run_train_persists_hybrid_pso_iteration_progress(monkeypatch) -> None:
    ...
    models_dir = raw_dir / "models"
    logs_dir = raw_dir / "logs"
    for path in [metrics_dir, metadata_dir, reports_dir, models_dir, logs_dir]:
        path.mkdir(parents=True, exist_ok=True)
