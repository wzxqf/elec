...
def test_prepare_project_context_exports_raw_schema_inputs_without_user_reports() -> None:
    output_root = Path(".cache") / "tests" / f"state-schema-{uuid4().hex}" / "outputs"
    config_path = _write_temp_config(output_root)
    context = prepare_project_context(PROJECT_ROOT, logger_name="test_prepare_context_state_schema", config_path=config_path)

    assert (context["output_paths"]["metadata"] / "compiled_parameter_layout.json").exists()
    assert (context["output_paths"]["metrics"] / "feature_manifest.csv").exists()
    report_names = {path.name for path in context["output_paths"]["reports"].iterdir()}
    assert "tensor_bundle_audit.md" not in report_names
    assert "state_schema_snapshot.md" not in report_names
