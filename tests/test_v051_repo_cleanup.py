...
ACTIVE_DOCS = [
    ...
    PROJECT_ROOT / f"{CURRENT_PROJECT_VERSION}.md",
    PROJECT_ROOT / "docs" / "ARCHITECTURE.md",
    PROJECT_ROOT / "docs" / "CONSTRAINTS.md",
    PROJECT_ROOT / "docs" / "STATE_SCHEMA.md",
    PROJECT_ROOT / "docs" / "agents.md",
    PROJECT_ROOT / "docs" / f"{CURRENT_PROJECT_VERSION}_architecture_implementation.md",
...
def test_param_opt_experiment_configs_are_not_active() -> None:
...
def test_obsolete_outputs_are_not_active() -> None:
...
def test_experiment_override_test_is_not_active() -> None:
    assert not (TESTS_ROOT / "test_v045_experiment_config_overrides.py").exists()


def test_pycache_is_ignored() -> None:
...
def test_current_architecture_doc_exists() -> None:
    assert (PROJECT_ROOT / "docs" / f"{CURRENT_PROJECT_VERSION}_architecture_implementation.md").exists()


def test_legacy_docs_are_not_active() -> None:
    gitignore = (PROJECT_ROOT / ".gitignore").read_text(encoding="utf-8")

    assert "已归档/" in gitignore
