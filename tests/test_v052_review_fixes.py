...
from src.scripts.common import _align_lt_price_metadata
...
def test_linked_lt_price_effective_value_feeds_training_tensor() -> None:
    weeks = pd.to_datetime(["2026-01-26", "2026-02-02"])
    metadata = pd.DataFrame(
        {
            "week_start": weeks,
            "forecast_weekly_net_demand_mwh": [100.0, 100.0],
            "actual_weekly_net_demand_mwh": [100.0, 100.0],
            "lt_price_w": [500.0, 438.1923],
            "da_price_mean": [500.0, 428.8434],
            "id_price_mean": [500.0, 411.5684],
            "lt_price_linked_active": [0.0, 1.0],
            "fixed_price_ratio_max": [0.4, 0.4],
            "linked_price_ratio_min": [0.6, 0.6],
        }
    )

    aligned = _align_lt_price_metadata(metadata)
    expected_linked_price = 0.4 * 428.8434 + 0.6 * 411.5684

    assert aligned.loc[0, "lt_price_w"] == 500.0
    assert aligned.loc[0, "lt_price_w_effective"] == 500.0
    assert aligned.loc[1, "lt_price_source"] == "linked_mix_40da_60id"
    assert aligned.loc[1, "lt_price_w_prev_week_da_proxy"] == 438.1923
    assert aligned.loc[1, "lt_price_w"] == pytest.approx(expected_linked_price)
    assert aligned.loc[1, "lt_price_w_effective"] == pytest.approx(expected_linked_price)

    bundle = {
        "weekly_features": pd.DataFrame({"week_start": weeks, "feature_a": [0.0, 0.0]}),
        "weekly_metadata": aligned,
        "policy_state_trace": pd.DataFrame({"week_start": weeks}),
        "hourly": pd.DataFrame(
            {
                "week_start": [weeks[0], weeks[1]],
                "hour_index": [0, 0],
                "net_load_da": [10.0, 10.0],
                "net_load_id": [10.0, 10.0],
                "price_spread": [0.0, 0.0],
                "load_dev": [0.0, 0.0],
                "renewable_dev": [0.0, 0.0],
            }
        ...
        "quarter": pd.DataFrame(
            {
                "week_start": [weeks[0], weeks[1]],
                "interval_index": [0, 0],
                "全网统一出清价格_日前": [500.0, 428.8434],
                "全网统一出清价格_日内": [500.0, 411.5684],
                "net_load_da_mwh": [100.0, 100.0],
                "net_load_id_mwh": [100.0, 100.0],
            }
        ),
        "agent_feature_columns": ["feature_a"],
    }
    ...
    assert tensor_bundle.lt_weekly_price.tolist() == pytest.approx([500.0, expected_linked_price])
