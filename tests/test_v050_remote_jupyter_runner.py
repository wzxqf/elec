...
def test_source_archive_excludes_local_outputs_caches_git_and_secret_files() -> None:
    ...
    try:
        ...
        (project_root / "tests" / "test_v050_example.py").write_text("def test_ok():\n    assert True\n", encoding="utf-8")
        (project_root / "outputs" / "v0.50").mkdir(parents=True)
        (project_root / "outputs" / "v0.50" / "local.csv").write_text("local\n", encoding="utf-8")
        ...
        assert "tests/test_v050_example.py" in names
        assert "outputs/v0.50/local.csv" not in names
...
def test_remote_execution_code_runs_pipeline_then_pytest_and_emits_result_marker() -> None:
    spec = run_remote_jupyter.RemoteRunSpec(
        run_id="v050-remote-test",
        version="v0.50",
        archive_remote_path="elec_remote_runs/v0.50/v050-remote-test/source.zip",
        remote_project_dir="elec_remote_runs/v0.50/v050-remote-test/project",
        artifact_remote_path="elec_remote_runs/v0.50/v050-remote-test/artifacts.zip",
        pytest_args=["tests/test_v050_repo_cleanup.py", "-q"],
        run_pipeline=True,
        run_tests=True,
    )
    ...
    assert '"tests/test_v050_repo_cleanup.py"' in code
    assert '"-q"' in code
...
def test_default_download_dir_is_versioned_remote_jupyter_subdirectory() -> None:
    ...
    download_dir = run_remote_jupyter.default_download_dir(project_root, "v0.50", "20260426T120000")

    assert download_dir == project_root / "outputs" / "v0.50" / "raw" / "remote_jupyter" / "20260426T120000"
...
def test_sync_pulled_outputs_replaces_local_formal_outputs_and_keeps_remote_history() -> None:
    ...
    try:
        project_root = temp_root / "project"
        target = project_root / "outputs" / "v0.50"
        ...
        pulled_output = remote_history / "outputs" / "v0.50"
        ...
        manifest = run_remote_jupyter.sync_pulled_outputs_to_local(remote_history, project_root, "v0.50")
...
def test_root_remote_jupyter_script_delegates_to_module_dry_run() -> None:
    ...
    assert "Remote root: elec_remote_runs/v0.50/root-wrapper-test" in result.stdout
