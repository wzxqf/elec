...
def test_dynamic_lock_only_uses_configured_baseline_ratio() -> None:
    tensor_bundle = TrainingTensorBundle(
        device="cpu",
        week_index=pd.Index(pd.to_datetime(["2026-02-02"])),
        weekly_feature_columns=[],
        ...
        weekly_feature_tensor=torch.zeros((1, 0), dtype=torch.float32),
        policy_tensor=torch.zeros((1, 0), dtype=torch.float32),
        hourly_tensor=torch.zeros((1, 2, 5), dtype=torch.float32),
        quarter_price_tensor=torch.tensor(
            [[[0.0, 10.0], [0.0, 10.0]]],
            dtype=torch.float32,
        ),
        ...
        hourly_bound_tensor=torch.zeros((1, 2, 0), dtype=torch.float32),
        forecast_weekly_load=torch.tensor([1000.0], dtype=torch.float32),
        actual_weekly_load=torch.tensor([1000.0], dtype=torch.float32),
        lt_weekly_price=torch.tensor([100.0], dtype=torch.float32),
        hourly_valid_mask=torch.tensor([[True, True]]),
        quarter_valid_mask=torch.tensor([[True, True]]),
    )
    ...
    result = evaluate_benchmark_strategies(
        {"tensor_bundle": tensor_bundle},
        {
            "score_kernel": {"baseline_position_ratio": 0.48},
            "economics": {"retail_tariff_yuan_per_mwh": 0.0},
        },
    )

    dynamic_cost = float(result.loc[result["strategy_name"] == "dynamic_lock_only", "total_procurement_cost"].iloc[0])
    assert dynamic_cost == 34000.0


def test_benchmark_strategies_use_real_15min_actual_need_when_available() -> None:
    tensor_bundle = TrainingTensorBundle(
        device="cpu",
        week_index=pd.Index(pd.to_datetime(["2026-02-02"])),
        weekly_feature_columns=[],
        ...
        hourly_tensor=torch.zeros((1, 2, 5), dtype=torch.float32),
        quarter_price_tensor=torch.tensor(
            [[[0.0, 1.0, 0.0, 10.0], [0.0, 100.0, 0.0, 90.0]]],
            dtype=torch.float32,
        ),
        weekly_bound_tensor=torch.zeros((1, 0), dtype=torch.float32),
        hourly_bound_tensor=torch.zeros((1, 2, 0), dtype=torch.float32),
        forecast_weekly_load=torch.tensor([0.0], dtype=torch.float32),
        actual_weekly_load=torch.tensor([100.0], dtype=torch.float32),
        lt_weekly_price=torch.tensor([0.0], dtype=torch.float32),
        hourly_valid_mask=torch.tensor([[True, True]]),
        quarter_valid_mask=torch.tensor([[True, True]]),
    )

    result = evaluate_benchmark_strategies({"tensor_bundle": tensor_bundle}, {"economics": {"retail_tariff_yuan_per_mwh": 0.0}})

    dynamic_cost = float(result.loc[result["strategy_name"] == "dynamic_lock_only", "total_procurement_cost"].iloc[0])
    assert dynamic_cost == 9010.0
