...
def test_obsolete_outputs_are_archived() -> None:
    ...
    assert (archived_outputs / "v0.45").exists()
    assert (archived_outputs / "v0.45-param-opt-explore").exists()
    assert (archived_outputs / "v0.46").exists()
    assert (archived_outputs / "v0.47").exists()
    ...
    assert not (PROJECT_ROOT / "outputs" / "v0.45").exists()
    assert not (PROJECT_ROOT / "outputs" / "v0.45-param-opt-explore").exists()
    assert not (PROJECT_ROOT / "outputs" / "v0.46").exists()
    assert not (PROJECT_ROOT / "outputs" / "v0.47").exists()
...
def test_current_architecture_doc_exists() -> None:
    assert (PROJECT_ROOT / "docs" / "v0.50_architecture_implementation.md").exists()
...
def test_legacy_docs_are_archived() -> None:
    ...
    assert (PROJECT_ROOT / "已归档" / "v0.45.md").exists()
    assert (PROJECT_ROOT / "已归档" / "v0.46.md").exists()
    assert (PROJECT_ROOT / "已归档" / "v0.47.md").exists()
    assert (archived_docs / "v0.45_architecture_implementation.md").exists()
    assert (archived_docs / "v0.46_architecture_implementation.md").exists()
    assert (archived_docs / "v0.47_architecture_implementation.md").exists()
    assert (archived_docs / "v0.47_algorithm_implementation_logic.md").exists()
    assert not (PROJECT_ROOT / "docs" / "V040_SPEC.md").exists()
    assert not (PROJECT_ROOT / "docs" / "superpowers").exists()
    assert not (PROJECT_ROOT / "v0.45.md").exists()
    assert not (PROJECT_ROOT / "v0.46.md").exists()
    assert not (PROJECT_ROOT / "v0.47.md").exists()
    assert not (PROJECT_ROOT / "docs" / "v0.45_architecture_implementation.md").exists()
    assert not (PROJECT_ROOT / "docs" / "v0.46_architecture_implementation.md").exists()
    assert not (PROJECT_ROOT / "docs" / "v0.47_architecture_implementation.md").exists()
    assert not (PROJECT_ROOT / "docs" / "v0.47_algorithm_implementation_logic.md").exists()
...
def test_current_output_dir_does_not_keep_transitional_version_labels() -> None:
    ...
    metadata_files = [
        ...
        output_root / "raw" / "metadata" / "train_config_snapshot.yaml",
    ]
    offenders = []
