...
面向湖南电力市场售电公司论文实验的多时间尺度采购策略工程。当前正式版本已按 `v0.42` 收口并增强为：
...
本版本在 `v0.4` 结构闭环基础上完成 `v0.42` 论文主版本收口，不改变正式策略边界：
...
- `<version>` 由 `experiment_config.yaml` 中的 `project.version` 自动决定，例如当前版本输出到 `outputs/v0.42/`。
