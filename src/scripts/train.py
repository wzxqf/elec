...
def build_train_summary(context: dict[str, Any], training: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## 训练结果",
            "",
            f"- 最终训练轮次: {config['hpso']['upper']['iterations'] if algorithm == 'HPSO' else config['total_timesteps']}",
            ...
            f"- 训练指标文件: {context['output_paths']['metrics'] / ('hpso_weekly_practice_data.csv' if algorithm == 'HPSO' else 'ppo_train_metrics.csv')}",
            f"- 评估指标文件: {context['output_paths']['metrics'] / ('hpso_convergence_curve.csv' if algorithm == 'HPSO' else 'ppo_eval_metrics.csv')}",
