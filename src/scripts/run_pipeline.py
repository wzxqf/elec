...
def _build_version_report(context: dict, training: dict, validation: dict, backtest: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "## 数学模型与公式",
            "",
            "- 上层周度决策变量: `a_w = (Δh_w^{raw}, b_w)`，其中 `Δh_w^{raw}` 表示相对强基准底仓的残差动作，`b_w` 表示现货可承担的边际敞口带宽。",
            "- 周度底仓生成关系: `lock_ratio_final = clip(lock_ratio_base + delta_lock_ratio, weekly_bounds)`，并保留 `lock_ratio_base`、`delta_lock_ratio`、`lock_ratio_final` 的审计口径。",
            "- 下层小时级修正变量: `delta_q_h = Proj(signal_spread, signal_load_dev, signal_renewable_dev | exposure_band, policy_bounds)`，不单独训练第二套 DRL。",
            "- 15 分钟代理结算口径: `C_t = q_lt,t * p_lt + q_spot,t * p_da + |q_real,t - q_sched,t| * p_id`。",
