...
def _build_run_summary(context: dict, training: dict, validation: dict, backtest: dict) -> str:
    ...
    lines.extend(
        [
            ...
            f"- 设备偏好: {config['device']}",
            f"- 训练设备: {training['device']}",
            f"- CUDA 可用: {training['device_info']['cuda_available']}",
            f"- CUDA 设备数: {training['device_info']['cuda_device_count']}",
            f"- MPS 可用: {training['device_info']['mps_available']}",
            "",
            "## Validation",
            "",
