...
## 1. 架构定位
本项目是围绕湖南电力市场售电公司采购策略实验构建的离线研究工程，当前已固化为“周度中长期持仓 PPO 决策 + 小时级显式规则现货修正 + 15 分钟代理结算回测”的两阶段架构，而不是端到端单层策略。程序目标不是搭建通用交易平台，而是为论文实证提供可复现的数据处理、训练、验证、回测与分析闭环。
...
### 3.1 configs/
统一管理实验配置，不允许在训练脚本和回测脚本中散落硬编码参数。
...
- `policy_parser`：解析政策目录中的 docx/md 文本
- `policy_regime`：生成周度制度状态轨迹
- `policy_tables`：输出结构化规则表与摘要报告
...
- 接收二维动作 `(lock_ratio, hedge_intensity)`
...
### 5.2 action
上层动作固定为二维：
- `lock_ratio`：下一周中长期锁定比例
- `hedge_intensity`：小时级现货修正强度

不再扩展成高维复杂动作空间，避免偏离当前论文规划。

### 5.3 reward
奖励不直接等于利润，而是对以下量的稳健惩罚组合：
- 相对动态基准的成本变化
- 风险项变化
- 交易成本
- 套保误差
...
- `backtest` 负责正式比较、图表导出、敏感性、鲁棒性和超参数搜索
